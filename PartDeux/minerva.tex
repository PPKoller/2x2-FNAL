...
\subsection{Repurposing MINERvA as a rock muon trigger}
In the discussion in Section~\ref{sec:minerva}, the detector setup maximizes the amount of the MINERvA detector repurposed downstream of the ArgonCube 2x2 module, in order to increase containment in the very forward region, where most particles go. However, if MINOS-ND is also included as part of the ProtoDUNE-ND setup, this imperative is no longer there, and less of MINERvA would be needed downstream. In this case, MINERvA modules could be redeployed to the sides of the 2x2 cryostat to act as a rock muon tagger, and to tag escaping particles. In practice, these panels could be used to better identify a sample of fully contained neutrino events. Additionally to the cosmic and rock muon suppression studies outlined in Ref.~\cite{2x2@FNAL}, these tagging modules can be used to calibrate the electric field uniformity. 

A calibrated electric field is vital for the ability to accurately measure $\frac{\mathrm{d}E}{\mathrm{d}x}$ within a TPC. The calibration requires crossing tracks with well defined start and end points~\cite{MicroBooNE_Laser}, covering the entire active argon volume.
12 MINERvA tracker modules upstream and downstream (24 total) of the 2x2 will provide sufficient resolution on the muon crossing position, and the ability to point in both directions. 
